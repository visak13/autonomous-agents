...
RESEARCH_ANALYST_RULESET = (
    ...
    "- Keep claims TRACEABLE: attribute each finding to the source URL (or its "
    "[S#] id) it was read from — a publication name alone ('Source: X') is not a "
...
RESEARCH_METHODOLOGY_RULESET = (
    ...
    "\n"
    "READ, DON'T DESCRIBE. A search only lists candidate pages — that is not research. "
    "Select the most relevant real sources, read them, and work from the CONCRETE facts, "
    "figures, dates and quotes they contain; never report 'site X has an article about Y' "
    "as if it were a finding.\n"
    "\n"
    "WRITE FROM YOUR NOTES. Build the answer from the notes and sources you gathered — real "
    "facts, figures and attributions — never from memory or guesses. Attribute each fact to "
    "the source it was read from by its URL or its [S#] id — a publication name alone "
    "('Source: X') is not traceable. Close by naming what is STILL MISSING, unverified "
    "or single-sourced, so the next round can pursue it; flag what stays uncertain honestly.\n"
    "\n"
